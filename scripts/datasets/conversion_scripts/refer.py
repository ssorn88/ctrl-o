...
class REFER:
    def __init__(self, data_root, dataset="refcoco", splitBy="unc"):
        ...
        if dataset in ["refcoco", "refcoco+", "refcocog"]:
            self.IMAGE_DIR = osp.join(data_root, "coco_raw/images/train2014")
        ...
        if seg_box == "seg":
            ...
            if type(ann["segmentation"][0]) == list:
                ...
                for seg in ann["segmentation"]:
                    ...
                    color.append(c)
                p = PatchCollection(
                    polygons, facecolors=color, edgecolors=(1, 1, 0, 0), linewidths=3, alpha=1
                )
                ax.add_collection(p)  # thick yellow polygon
                p = PatchCollection(
                    polygons, facecolors=color, edgecolors=(1, 0, 0, 0), linewidths=1, alpha=1
                )
