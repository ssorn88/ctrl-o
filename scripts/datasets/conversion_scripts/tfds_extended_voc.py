...
# Data mirror (note that test split appears to be only available on those)
_VOC_DATA_URL_MIRROR_1 = "http://pjreddie.com/media/files/"
_VOC_DATA_URL_MIRROR_2 = "https://data.brainchip.com/dataset-mirror/voc/"
_SBD_DATA_URL = "https://www2.eecs.berkeley.edu/Research/Projects/CS/vision/grouping/"
...
class VocConfig(tfds.core.BuilderConfig):
    ...
    def __init__(self, year, has_segmentation, splits, **kwargs):
        ...
        if year == 2007:
            self.filenames = {
                "trainval": os.path.join(
                    _VOC_DATA_URL.format(year=year), "VOCtrainval_06-Nov-2007.tar"
                ),
                "test": os.path.join(_VOC_DATA_URL_MIRROR_2, "VOCtest_06-Nov-2007.tar"),
            }
        else:
            self.filenames = {
                "trainval": os.path.join(
                    _VOC_DATA_URL_MIRROR_2.format(year=year), "VOCtrainval_11-May-2012.tar"
                ),
                "test": os.path.join(_VOC_DATA_URL_MIRROR_2, "VOC2012test.tar"),
            }
