...
class WebdatasetDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        use_model_transform: bool = False,  # <--- 파라미터 추가
        transform_from_model=None,  # <--- train.py에서 동적으로 주입되는 파라미터
    ):
        ...
        Args:
            ...
            use_epochs: If `True`, dataloader stops after reaching one epoch as defined by
                `train_size`. If `False`, dataloader yields an infinite stream of batches.
        ...
        self.use_model_transform = use_model_transform
        self.transform_from_model = transform_from_model
    ...
    def _create_webdataset(
        ...
        if self.use_epochs and n_datapoints is not None:
            return torchdata.datapipes.iter.LengthSetter(datapipe, n_datapoints)
        elif not shuffle:
            # During validation, do not cycle infinitely.
            return datapipe
        else:
    ...
    if isinstance(elem, torch.Tensor):
        ...
        try:
            ...
            if not all(x.ndim == elem.ndim for x in batch):
                dims = [x.ndim for x in batch]
                raise ValueError(
                    f"Tensors in batch have different number of dimensions {dims}. Example: {batch}"
                )
