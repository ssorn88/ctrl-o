...
class EvaluationConfig:
    ...
    train_config_path: str = "/home/mila/a/aniket.didolkar/scratch/language_conditioned_oclf/outputs/projects/prompting/vg/prompt_vg_small14_dinov2_mapping_lang_point_pred_sep/2024-10-02_02-13-27/config/config.yaml"
    ...
    checkpoint_path: Optional[
        str
    ] = "/home/mila/a/aniket.didolkar/scratch/language_conditioned_oclf/outputs/projects/prompting/vg/prompt_vg_small14_dinov2_mapping_lang_point_pred_sep/2024-10-02_02-13-27/checkpoints/epoch=0-step=111000.ckpt"
    output_dir: Optional[str] = "coco"
    ...
    # Add callbacks for evaluation
    callbacks: Optional[Dict[str, Any]] = None
    # Dictionary with arbitrary additional information.
    experiment: Dict[str, Any] = dataclasses.field(default_factory=dict)
    # Callable object that takes state dict of type Dict[str, Any] and modifies it in place.
...
def evaluate(config: EvaluationConfig):
    ...
    if config.checkpoint_hook is not None:
        checkpoint_hook = hydra_zen.instantiate(config.checkpoint_hook, _convert_="all")
    else:
    ...
    with hydra.initialize_config_dir(config_dir=config_dir):
        ...
        datamodule, model = eval_utils.build_from_train_config(
            ...
            datamodule_kwargs=dict(eval_batch_size=config.eval_batch_size),
    ...
    if config.callbacks is not None:
        callbacks = list(hydra_zen.instantiate(config.callbacks, _convert_="all").values())
    else:
        callbacks = []
    ...
    if config.save_outputs:
        ...
        callbacks.append(data_extractor)
