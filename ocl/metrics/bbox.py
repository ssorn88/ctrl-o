...
class UnsupervisedBboxIoUMetric(torchmetrics.Metric):
    ...
    def __init__(
        ...
        matchings = ("hungarian", "best_overlap", "gt")
    ...
    def update(
        self, prediction: torch.Tensor, target: torch.Tensor, gt_indices: torch.Tensor = None
    ):
        """Update this metric.
        ...
        for pred, target in zip(pred_bboxes, target_bboxes):
            ...
            if valid_pred_bboxes.sum() > 0:
                iou_per_bbox = unsupervised_bbox_iou(
                    pred, target, matching=self.matching, gt_indices=gt_indices, reduction="none"
                )
            else:
...
def unsupervised_bbox_iou(
    ...
    if matching == "hungarian":
        ...
        if gt_indices is None:
            raise ValueError("gt_indices must be provided when using matching='gt'.")
        pred_idxs = []
