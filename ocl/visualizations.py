...
def _align_masks_to_gt(prediction, target, ignore=None, selected_indices=None):
    if target.shape[-1] == 1:
    ...
    for pred, pred_orig, target, target_empty, indices in zip(
        ...
        true_idxs_list.append(true_idxs_t)

    return true_idxs_list
...
class Image(VisualizationMethod):
    ...
    def __init__(
        ...
        Args:
            ...
            reverse_channels: Reverse the channels of the image
        """
...
class SegmentationWithText(VisualizationMethod):
    """Segmentaiton visualization."""
    ...
    def __call__(
        ...
        if image.dim() == 4:  # Only support image data at the moment.
            ...
            true_idxs_list = _align_masks_to_gt(
                mask.to(gt_masks.device), gt_masks, selected_indices=selected_indices
            )

            bboxs = []
            ignore_bboxs = []
            correct = []
            for i, true_idxs in enumerate(true_idxs_list):
                this_img_bboxs = []
                this_img_ignore_bboxs = []
                this_correct = []
                for j, true_idx in enumerate(true_idxs):
                    bbox = bbox_centroids[i, true_idx]
                    this_img_bboxs.append(bbox)

                    if selected_indices[i, j] == -1:
                        this_img_ignore_bboxs.append(0)
                    else:
                        this_img_ignore_bboxs.append(1)

                    if true_idx == selected_indices[i, j]:
                        this_correct.append(1)
                    else:
                        this_correct.append(0)

                bboxs.append(this_img_bboxs)
                ignore_bboxs.append(this_img_ignore_bboxs)
            ...
            for i, bbox in enumerate(bboxs):
                for j in range(len(bbox)):
                    x, y = bbox[j]
                    if ignore_bboxs[i][j] == 1:
                        np_image = masks_on_image[i].permute(1, 2, 0).numpy().copy()
                        np_image = np_image.astype(np.uint8)
                        masks_on_image[i] = torch.from_numpy(
                            cv2.putText(
                                np_image,
                                text[j][i],
                                (int(x), int(y)),
                                ...
                                0.5,
                                (0, 255, 0) if correct[i][j] == 1 else (255, 0, 0),
                                2,
                                cv2.LINE_AA,
                            )
                        ).permute(2, 0, 1)
