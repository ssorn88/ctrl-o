...
class ApplyModelImageTransform:
    """
    OpenVLA(또는 외부 모델)에서 제공하는 고유의 image_transform을
    ctrl-o의 데이터 파이프라인에 적용하기 위한 래퍼 클래스입니다.
    """

    def __init__(self, transform=None, image_key: str = "image"):
        """
        Args:
            transform: 모델에서 추출한 전처리 함수 (예: OpenVLA의 image_transform)
            image_key: 데이터 딕셔너리에서 이미지가 저장된 키 이름
        """
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 외부 transform이 주입되지 않았다면 원본 데이터를 그대로 반환
        if self.transform is None:
            return data

        img = data[self.image_key]

        # OpenVLA(Prismatic)의 transform은 보통 PIL Image를 입력으로 기대합니다.
        # 따라서 numpy array나 Tensor인 경우 PIL Image로 변환해 줍니다.
        if isinstance(img, numpy.ndarray):
            # numpy array가 (H, W, C) 형태라고 가정
            # 만약 grayscale(H, W, 1)이거나 흑백(H, W)라면 3채널 RGB로 변환 필요
            if img.ndim == 2:
            ...
            # 텐서인 경우 (C, H, W) 형태라고 가정하고 변환
            if img.ndim == 4:  # (1, C, H, W)인 경우
                img = img.squeeze(0)
            img = F.to_pil_image(img)

        # 3. OpenVLA transform 적용 (내부적으로 Resize, ToTensor, Normalize 수행)
        transformed_img = self.transform(img)

        # 4. 변환된 텐서를 다시 데이터 딕셔너리에 저장
        data[self.image_key] = transformed_img
...
class SelectConditioningInfo:
    """Select conditioning information from data dictionary."""

    def __init__(self, num_max_binds: int = 3, num_slots: int = 33, is_refcocog: bool = False):
        """Initialize SelectConditioningInfo.

        Args:
            keys: Entries that should be selected from the input dict.
        """
        self.num_max_binds = num_max_binds
        self.dataset_prefix = os.environ["DATASET_PREFIX"]
        self.embeddings_path = os.path.join(self.dataset_prefix, "category_name_to_llama3_emb.pkl")
        self.num_slots = num_slots
        self.is_refcocog = is_refcocog
        # Check if the file already exists
        if not os.path.exists(self.embeddings_path):
            # Download the file if it does not exist
            url = "https://huggingface.co/andriizadaianchuk/coco_llama/resolve/main/category_name_to_llama3_emb.pkl"
            response = requests.get(url)
            response.raise_for_status()  # Raises an HTTPError for bad responses

            # Save the file locally
            with open(self.embeddings_path, "wb") as file_:
                file_.write(response.content)
            print(f"Downloaded and saved {self.embeddings_path}.")

        # Load the embeddings using pickle
        with open(self.embeddings_path, "rb") as file_:
            self.embeddings = pickle.load(file_)
    ...
    def __call__(self, data: Dict[str, Any]):
        ...
        selected_indices_dtype = selected_indices.dtype
        bbox_centroids = data["bbox_centroids"]
        bbox_centroids_dtype = bbox_centroids.dtype
        contrastive_mask_loss = data["contrastive_loss_mask"]
        contrastive_mask_loss_dtype = contrastive_mask_loss.dtype
        name_embedding = data["name_embedding"]
        name_embedding_dtype = name_embedding.dtype
        if self.is_refcocog:
            references_embedding = data["references_embedding"]
            references_embedding_dtype = references_embedding.dtype
        # category_idx = numpy.array([self.key_to_idx[names[i]] for i in range(initial_num)])
        ...
        for i in range(initial_num):
            if selected_indices[i] == -1:
                break
        valid_cnt = i
        valid_cnt = min(valid_cnt, self.num_slots)
        assert (
        ...
        for j, i in enumerate(inds):
            new_names[j] = names[i]
            new_name_embedding[j] = name_embedding[i]
            # new_category_idx[j] = self.key_to_idx[names[i]]
            new_selected_indices[j] = selected_indices[i]
            new_bbox_centroids[j] = bbox_centroids[i]  # / 224
            new_contrastive_mask_loss[j] = contrastive_mask_loss[i]
            if self.is_refcocog:
                new_references_embedding[j] = references_embedding[i]

        data["name"] = new_names
        data["selected_indices"] = new_selected_indices.astype(selected_indices_dtype)

        data["bbox_centroids"] = new_bbox_centroids.astype(bbox_centroids_dtype) / 224
        data["contrastive_loss_mask"] = new_contrastive_mask_loss.astype(contrastive_mask_loss_dtype)
        data["contrastive_loss"] = numpy.zeros_like(data["contrastive_loss_mask"])
        data["name_embedding"] = new_name_embedding.astype(name_embedding_dtype)
        if self.is_refcocog:
            data["references_embedding"] = new_references_embedding.astype(
                references_embedding_dtype
            )
...
def resize_bboxes(bboxes, original_size, new_size):
    ...
    return numpy.array(resized_bboxes)

def resize_centroids(centroids, original_size, new_size):
    """
    Resize bounding boxes according to the new image size.
    ...
    resized_centroids = []
    for bbox in centroids:
        x, y = bbox
        x_new = int(x * scale_y)
        y_new = int(y * scale_x)
        resized_centroids.append([x_new, y_new])

    return numpy.array(resized_centroids)
...
def resize_masks(masks, new_size):
    ...
    """
    if masks.ndim == 2:
        return cv2.resize(masks, new_size, interpolation=cv2.INTER_NEAREST)
    else:
        masks = masks[:, :, :, 0]
        resize_masks = []
        for mask in masks:
            resize_masks.append(cv2.resize(mask, new_size, interpolation=cv2.INTER_NEAREST))
        return numpy.stack(resize_masks, axis = 0)[:, :, :, None]

def bboxes_to_masks(bboxes, image_shape):
    """
    Convert bounding boxes to instance masks.

    Args:
        bboxes (np.ndarray): Array of bounding boxes of shape (N, 4), where each box is [x_min, y_min, x_max, y_max].
        image_shape (tuple): Shape of the original image as (height, width).

    Returns:
        np.ndarray: Instance masks of shape (N, height, width).
    """
    N = bboxes.shape[0]
    height, width = image_shape
    masks = numpy.zeros((N, height, width), dtype=numpy.uint8)

    for i, (x_min, y_min, x_max, y_max) in enumerate(bboxes):
        # Draw a filled rectangle on the mask for each bounding box
        masks[i, int(y_min):int(y_max), int(x_min):int(x_max)] = 1.0

    return masks



class SelectConditioningInfoVG:
    """Select conditioning information from data dictionary."""

    def __init__(self, num_max_binds: int = 3, num_slots: int = 7):
        """Initialize SelectConditioningInfo.
    ...
    def __call__(self, data: Dict[str, Any]):
        ...
        new_instance_bbox = numpy.zeros((num_slots, 4), dtype=numpy.float32)

        if initial_num < binds:
            for i in range(initial_num):
                new_names[i] = names[i]
                new_name_embedding[i] = name_embedding[i]
                new_selected_indices[i] = i
                new_bbox_centroids[i] = bbox_centroids[i]
                new_contrastive_mask_loss[i] = 1
                new_instance_bbox[i] = instance_bbox[i]
        else:
            if "selected_indices" in data:
                valid_cnt = 0
                for i in range(initial_num):
                    if data["selected_indices"][i] == -1:
                        break
                    valid_cnt = i + 1
            ...
            for j, i in enumerate(inds):
                ...
                new_selected_indices[j] = i
                new_bbox_centroids[j] = bbox_centroids[i]  # / 224
                new_contrastive_mask_loss[j] = 1
                new_instance_bbox[j] = instance_bbox[i]
        ...
        if img.ndim == 2:
            img = numpy.concatenate((img[:, :, None], img[:, :, None], img[:, :, None]), axis=-1)
        #data["image"] = cv2.resize(img, (518, 518), interpolation=cv2.INTER_LINEAR)
        #new_bbox_centroids = resize_centroids(new_bbox_centroids, (224, 224), (518, 518)) / 518
        #new_instance_bbox = resize_bboxes(new_instance_bbox, (224, 224), (518, 518))
        new_instance_bbox_mask = bboxes_to_masks(new_instance_bbox, (224, 224))
        new_instance_mask = data["instance_mask"]


        # save image with bboxes and centroids
        """
        import cv2
        import numpy as np
        
        i = random.randint(0, len(new_instance_bbox))
        print(new_names[i])
        x1, y1, x2, y2 = new_instance_bbox[i]
        x1, y1, x2, y2 = int(x1), int(y1), int(x2), int(y2)
        img = cv2.rectangle(img, (x1, y1), (x2, y2), (0, 255, 0), 2)
        x, y = new_bbox_centroids[i]
        x, y = int(x), int(y)
        img = cv2.circle(img, (x, y), 5, (0, 0, 255), -1)
        r = random.randint(0, 100000)
        cv2.imwrite(f"/home/mila/a/aniket.didolkar/scratch/lang_oclf_github/language_conditioned_oclf/check_instance_bbox_{r}.jpg", img)
        cv2.imwrite(f"/home/mila/a/aniket.didolkar/scratch/lang_oclf_github/language_conditioned_oclf/check_instance_bbox_mask_{r}.jpg", new_instance_mask * 255.0)
        exit()
        """
        
        data["image"] = img
        data["selected_indices"] = new_selected_indices
        data["bbox_centroids"] = new_bbox_centroids.astype(numpy.float32) / 224
        data["instance_bbox"] = new_instance_bbox.astype(numpy.float32) / 224
        data["instance_bbox_mask"] = new_instance_bbox_mask.astype(numpy.float32)
        data["instance_mask"] = new_instance_mask
        data["contrastive_loss_mask"] = new_contrastive_mask_loss
        data["name_embedding"] = new_name_embedding.astype(name_embedding_dtype)
...
class AddEmptyMasksVG:
    ...
    def __call__(self, array: numpy.ndarray) -> numpy.ndarray:
        ...
        if array.shape[0] < 10:
            pad = numpy.zeros(
                (10 - array.shape[0], array.shape[1], array.shape[2]), dtype=numpy.uint8
            )
            array = numpy.concatenate((array, pad), axis=0)
        return array
...
class DenseMaskToTensor:
    ...
    def __call__(self, mask: numpy.ndarray) -> torch.Tensor:
        # assert mask.shape[-1] == 1, f"Expected mask to have shape (..., K, H, W, 1), but got {mask.shape}"
...
class AddIgnoreMasks:
    """Add ignore masks covering empty pixels of given mask."""

    def __init__(self, mask_key: str, ignore_key: str = "ignore_mask"):
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        ignore_mask = masks.sum(0) == 0  # Sum over class dimension
        data[self.ignore_key] = ignore_mask[None].astype(numpy.uint8)
...
class InstanceMasksToDenseMasks:
    ...
    def convert(instance_mask: numpy.ndarray, categories: numpy.ndarray) -> numpy.ndarray:
        if numpy.min(categories) <= 0:
            m_cat = numpy.min(categories)
            raise ValueError(
                f"Detected category smaller equal than 0 in instance masks. Min category: {m_cat}"
            )
        ...
        mask = (instance_mask * categories).astype(numpy.uint8)
        assert mask.ndim == 4
        return mask
...
class DropCrowdMasks:
    """Drop masks corresponding to crowds."""

    def __init__(
        self,
        mask_key: str = "instance_mask",
        crowd_key: str = "instance_iscrowd",
        target_key: Optional[str] = None,
        missing_okay: bool = False,
    ):
        self.mask_key = mask_key
        self.crowd_key = crowd_key
        self.target_key = target_key if target_key else mask_key
        self.missing_okay = missing_okay

    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        if self.mask_key not in data:
            if self.missing_okay:
                return data
            raise ValueError(
                f"Masks not found under key {self.mask_key} in data dict. "
                f"Available options: {','.join(data)}"
            )
        if self.crowd_key not in data:
            raise ValueError(
                f"iscrowd not found under key {self.crowd_key} in data dict. "
                f"Available options: {','.join(data)}"
            )

        masks = data[self.mask_key]
        iscrowd = data[self.crowd_key].astype(bool)
        if len(masks) != len(iscrowd):
            raise ValueError(
                f"Masks and iscrowd have different lengths: {len(masks)} " f"vs {len(iscrowd)}"
            )

        masks = masks[~iscrowd]
        data[self.target_key] = masks
...
class ToGrayscale:
    ...
    def __call__(self, image: numpy.ndarray) -> numpy.ndarray:
        ...
        return image.mean(axis=-1, keepdims=True).astype(image.dtype)
...
class ResizeNearestExact:
    ...
    def resize(img: torch.Tensor, size: Union[int, List[int]], max_size: Optional[int] = None):
        ...
        img = torch.nn.functional.interpolate(
            img, size=[new_h, new_w], mode="nearest-exact", antialias=False
        )
...
class JointRandomResizedCropwithParameters(transforms.RandomResizedCrop):
    ...
    def forward(self, img: torch.Tensor, masks: Optional[Dict] = None) -> torch.Tensor:
        ...
        img = transforms.functional.resized_crop(
            img, *params, self.size, self.interpolation, antialias=True
        )
