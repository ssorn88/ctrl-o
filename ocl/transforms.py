...
class Filter(Transform):
    ...
    def __init__(
        self, predicate: Callable[..., bool], fields: Sequence[str], missing_okay: bool = False
    ):
        ...
        Args:
            ...
            missing_okay: If true, fields which are missing are replaced with `None`.
        """
    ...
    def _filter_using_predicate(self, d: Dict[str, Any]):
        if self.missing_okay:
            return self.predicate(*(d.get(field) for field in self._fields))
        else:
