...
Slightly adapted to conform to linters.

From https://gitlab.com/ericvsmith/toposort/.
"""
#######################################################################
...
class CircularDependencyError(ValueError):
    def __init__(self, data):
        deps = {", ".join(f"{key}:{value}" for key, value in data.items())}
        s = f"Circular dependencies exist among these items: {deps}"
...
def toposort_flatten(data, sort=True):
    """Topological sort returning flat list.
