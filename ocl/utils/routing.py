...
class Combined(nn.ModuleDict):
    ...
    For routed classes (`routed.RoutedClass`, `RoutableMixin`), the execution order of the modules
    will be automatically determined by the input dependencies between modules. For other classes
    (`Combined`, `Recurrent`), this is not implemented, and the user needs to take care to correctly
    order those classes.
    """

    def __init__(
        self, **modules: Dict[str, Union[routed.RoutedClass, RoutableMixin, Combined, Recurrent]]
    ):
        # Construct dependency graph of modules.
        module_dependencies: Dict[str, Set[str]] = {}  # Modules to parent modules
        for name, module in modules.items():
            if isinstance(module, (routed.RoutedClass, RoutableMixin)):
                module_dependencies[name] = {path[0] for path in module.input_mapping.values()}
            else:
                module_dependencies[name] = set()

        # Sort modules by topological order of dependency graph.
        orig_module_order = {name: idx for idx, name in enumerate(modules)}
        modules_ordered = {}
        for group in ocl.utils.toposort.toposort(module_dependencies):
            # Remove dependencies not in the input set of modules as they are outside our control.
            group = [name for name in group if name in modules]
            # Sort each group by their order in the original input.
            group = sorted(group, key=lambda k: orig_module_order[k])
            # Add module to new module dictionary.
            for name in group:
                modules_ordered[name] = modules[name]

        assert set(modules) == set(modules_ordered)  # Make sure we have not lost a module.
        super().__init__(modules_ordered)
