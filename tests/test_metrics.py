...
def test_log_tensor(reduction):
    ...
    tensor2 = torch.tensor([[1, 4, 3], [-2, 1, 5], [30, 1, 2]], dtype=torch.float32)
...
def test_unsupervised_mask_iou_threshold(assert_tensors_equal):
    def make_tensors(tensor1, tensor2):
        return torch.tensor(tensor1, dtype=torch.float64), torch.tensor(tensor2, dtype=torch.float64)
    ...
    assert_tensors_equal(
        ...
        make_tensors([1.0, 1.0, 0.0], [1.0, 1.0, 0.0]),
    )
    ...
    assert_tensors_equal(
        ...
        make_tensors([1.0, 1.0], [1.0, 0.0, 1.0]),
    )

    # Overlap below threshold for some classes.
    assert_tensors_equal(
        ocl.metrics.masks.unsupervised_mask_iou(
            _make_one_hot([0, 0, 0, 1, 1, 2], 4).transpose(0, 1),
            _make_one_hot([0, 0, 1, 2, 2, 2], 3).transpose(0, 1),
            matching="threshold",
            reduction="none",
        ),
        make_tensors([2 / 3, 0.0, 2 / 3], [2 / 3, 2 / 3, 0.0, 0.0]),
    )
...
def test_unsupervised_mask_iou_metric_pq():
    metric = metrics.PanopticQuality()

    # Perfect match
    pred = _make_one_hot([[1, 1, 0, 0], [0, 0, 0, 1]], 2)
    target = _make_one_hot([[1, 1, 0, 0], [0, 0, 0, 1]], 2)
    metric.update(
        pred.permute(0, 2, 1).unflatten(-1, (2, 2)),
        target.permute(0, 2, 1).unflatten(-1, (2, 2)),
    )
    ...
    metric.reset()
    # 3 TP, 3 FP, 1 FN
    pred = _make_one_hot([[0, 0, 1, 2], [0, 0, 0, 1]], 3)
    target = _make_one_hot([[0, 0, 0, 1], [0, 0, 1, 1]], 2)
    metric.update(
        pred.permute(0, 2, 1).unflatten(-1, (2, 2)),
        target.permute(0, 2, 1).unflatten(-1, (2, 2)),
    )
    target = (2 * (2 / 3) + 1.0) / (3 + 0.5 * 3 + 0.5 * 1)
    assert torch.allclose(metric.compute(), torch.tensor(target, dtype=torch.float64))
