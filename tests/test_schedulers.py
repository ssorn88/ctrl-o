...
@pytest.mark.parametrize("warmup_type", ["linear", "cosine"])
def test_cosine_annealing_hp_scheduler(warmup_type):
    start_step, warmup_steps, end_step = 10, 10, 30
    start_value, warmup_value, end_value = 1.0, 0.0, 1.0
    sched = scheduling.CosineAnnealingHPScheduler(
    ...
    sched.update_global_step(start_step + warmup_steps // 2)
    assert float(sched) == 0.5

    sched.update_global_step(start_step + warmup_steps)
    assert float(sched) == warmup_value

    sched.update_global_step(start_step + (end_step - start_step - warmup_steps) // 2)
    assert float(sched) == 0.5
