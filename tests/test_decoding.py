...
@pytest.mark.parametrize("top_k", [1, 2, [2, 3]])
@pytest.mark.parametrize(
    ["training_top_k", "eval_top_k"], [[False, True], [True, False], [True, True]]
)
def test_patch_decoder_top_k(top_k, training_top_k, eval_top_k):
    batch_size, n_objects, object_dim, feature_dim, n_patches = 2, 3, 5, 10, 8
    decoder = decoding.PatchDecoder(
        object_dim,
        feature_dim,
        n_patches,
        decoder=functools.partial(neural_networks.build_mlp, features=[20, 10]),
        top_k=3,
        training_top_k=training_top_k,
    )

    slots = torch.randn(batch_size, n_objects, object_dim)
    masks = torch.randn(batch_size, n_objects, n_patches).softmax(1)

    if training_top_k:
        result = decoder(slots, masks=masks)
        assert result.reconstruction.shape == (batch_size, n_patches, feature_dim)
        assert result.masks.shape == (batch_size, n_objects, n_patches)
